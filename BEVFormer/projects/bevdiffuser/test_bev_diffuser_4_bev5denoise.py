...
def evaluate(admm_denoiser,
    ...
    nusc = NuScenes(version='v1.0-trainval', dataroot='/root/autodl-fs/nuscenes', verbose=True)

    for step, batch in enumerate(dataloader):
        vis_ls = []  # 初始BEV/加噪BEV/cond最后一轮输出/调度器最终计算输出
        vis_ls2 = []  # cond的头四次输出
        vis_ls3 = []  # uncond最后一轮输出4张一样
        vis_ls4 = []  # uncond的头四次输出
        vis_ls5 = []  # uncond+cond最后一轮输出4张一样
        vis_ls6 = []  # uncond+cond的头四次输出
        TARGET_SAMPLE_TOKEN = batch['img_metas'][0].data[0][0]['sample_idx']

        if TARGET_SAMPLE_TOKEN != '340a5f3d8c4d4f11ae02176c86bfbad8':
            if rank == 0:
                prog_bar.update()
            continue
        ...
        if denoise_timesteps > 0:
            ...
            for idx, t in enumerate(noise_scheduler.timesteps): #5，4，3，2，1
                ...
                # 要内部迭代图用这个
                if t == 1:
                    noise_pred_uncond, ls_x_uncond, ls_z_uncond = admm_denoiser(latents, **denoiser_kwargs, last_scheduler_iter = True, **uncond)
                    noise_pred_cond, ls_x_cond, ls_z_cond = admm_denoiser(latents, **denoiser_kwargs, last_scheduler_iter = True, **cond)
                    noise_pred = noise_pred_uncond + 2 * (noise_pred_cond - noise_pred_uncond)
                    ...
                    vis_ls3 = ls_x_cond
                    vis_ls4 = ls_z_cond
                    for x_uncond, x_cond in zip(ls_x_uncond, ls_x_cond):
                        vis_ls5.append(x_uncond + 2 * (x_cond - x_uncond))
                    for z_uncond, z_cond in zip(ls_z_uncond, ls_z_cond):
                        vis_ls6.append(z_uncond + 2 * (z_cond - z_uncond))

                # 要外围图解禁这个
                # if t == 1:
                    # vis_ls.append(noise_pred_cond.clone().detach().cpu())
                    # vis_ls3.append(noise_pred_uncond.clone().detach().cpu())
                    # vis_ls3.append(noise_pred_uncond.clone().detach().cpu())
                    # vis_ls3.append(noise_pred_uncond.clone().detach().cpu())
                    # vis_ls3.append(noise_pred_uncond.clone().detach().cpu())
                    # vis_ls5.append(noise_pred.clone().detach().cpu())
                    # vis_ls5.append(noise_pred.clone().detach().cpu())
                    # vis_ls5.append(noise_pred.clone().detach().cpu())
                    # vis_ls5.append(noise_pred.clone().detach().cpu())
                # 要内部迭代图用这个
                ...
                # 要外围图解禁这个
                # else:
                    # vis_ls2.append(noise_pred_cond.clone().detach().cpu())
                    # vis_ls4.append(noise_pred_uncond.clone().detach().cpu())
                    # vis_ls6.append(noise_pred.clone().detach().cpu())
            ...
            # 可视化内部ADMM迭代结果
            vis_dir = f'/tmp/pycharm_project_55/BEVFormer/tools/vis_doc/vis_results_{TARGET_SAMPLE_TOKEN}'
            ...
            if vis_ls:
                ...
                lidiar_render_gt_simple(dataset=nusc, sample_token=TARGET_SAMPLE_TOKEN, out_path=None)
                print("Are U satisfy?")
        if rank == 0:
            prog_bar.update()
        break
...
def visualize_bev_features(feat_ls_1, feat_ls_2, feat_ls_3, feat_ls_4, feat_ls_5, feat_ls_6, save_dir=None, step_names=None):
    """
    ...
    feat_ls_list = [feat_ls_1, feat_ls_2, feat_ls_3, feat_ls_4, feat_ls_5, feat_ls_6]
    ...
    for hang, (var_name, f_l) in enumerate(zip(row_names, feat_ls_list)):

        # 确保vis_ls包含3个特征图
        assert len(f_l) == 4, f"Expected 4 feature maps, got {len(f_l)}"

        for lie, feat_in_f_l in enumerate(f_l):
            if isinstance(feat_in_f_l, torch.Tensor):
                feat_in_f_l = feat_in_f_l.numpy()

            # 计算统计量
            mean_val = np.mean(feat_in_f_l)
            median_val = np.median(feat_in_f_l)
            var_val = np.var(feat_in_f_l)
            std_val = np.std(feat_in_f_l)
            min_val = np.min(feat_in_f_l)
            max_val = np.max(feat_in_f_l)

            # 稀疏度（接近0的比例）
            sparsity = (np.abs(feat_in_f_l) < 1e-6).mean() * 100

            # 打印统计信息
            print(f"{var_name:<20} {lie + 1:<6} {mean_val:<15.6f} {median_val:<15.6f} "
                  f"{var_val:<15.6f} {std_val:<15.6f} {min_val:<15.6f} {max_val:<15.6f} {sparsity:<15.2f}%")
            ...
            im = axes[hang][lie].imshow(feat_in_f_l_mean, cmap='gray', aspect='auto', interpolation='bicubic')
            axes[hang][lie].set_title(f'{row_names[hang]} - Iter {lie+1}', fontsize=10)
            axes[hang][lie].axis('off')

            # 添加colorbar，显示强度值范围

            plt.colorbar(im, ax=axes[hang][lie], fraction=0.046, pad=0.04)
    ...
    plt.tight_layout()
    save_path = os.path.join(save_dir, f'bevdiffuser_stage1_inside_zhijie_bicubic_upsidown.png')
