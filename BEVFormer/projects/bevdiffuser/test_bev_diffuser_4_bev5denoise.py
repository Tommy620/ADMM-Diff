...
def evaluate(
    ...
    nusc = NuScenes(
        version="v1.0-trainval", dataroot="/root/autodl-fs/nuscenes", verbose=True
    )
    ...
    for step, batch in enumerate(dataloader):
        ...
        if denoise_timesteps > 0:
            ...
            for idx, t in enumerate(noise_scheduler.timesteps):  # 5，4，3，2，1
                ...
                # 要内部迭代图用这个
                if t == 1:
                    noise_pred_uncond, ls_x_uncond, ls_z_uncond = admm_denoiser(
                        latents, **denoiser_kwargs, last_scheduler_iter=True, **uncond
                    )
                    noise_pred_cond, ls_x_cond, ls_z_cond = admm_denoiser(
                        latents, **denoiser_kwargs, last_scheduler_iter=True, **cond
                    )
                ...
                # 要内部迭代图用这个
                    ...
                    noise_pred_uncond = admm_denoiser(
            ...
            if vis_ls:
                ...
                lidiar_render_gt_simple(
                    dataset=nusc, sample_token=TARGET_SAMPLE_TOKEN, out_path=None
                )
                print("Are U satisfy?")
        if rank == 0:
            prog_bar.update()
...
def visualize_bev_features(
    ...
    for hang, (var_name, f_l) in enumerate(zip(row_names, feat_ls_list)):
        ...
        for lie, feat_in_f_l in enumerate(f_l):
            ...
            # 打印统计信息
            print(
                f"{var_name:<20} {lie + 1:<6} {mean_val:<15.6f} {median_val:<15.6f} "
            ...
            im = axes[hang][lie].imshow(
                feat_in_f_l_mean, cmap="gray", aspect="auto", interpolation="bicubic"
            )
    ...
    plt.tight_layout()
    save_path = os.path.join(
        save_dir, f"bevdiffuser_stage1_inside_zhijie_bicubic_upsidown.png"
    )
...
def lidiar_render_gt_simple(dataset, sample_token, out_path=None):
    ...
    # 调用可视化函数，只显示GT
    visualize_sample(
        dataset, sample_token, gt_annotations, pred_annotations, savepath=out_path
    )
