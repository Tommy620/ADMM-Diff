...
def evaluate(admm_denoiser,
    ...
    for step, batch in enumerate(dataloader):
        ...
        if enable_timing:
            ...
            if timing_started and timing_sample_count >= timing_num_samples:
                print(f"[Table 8] Reached {timing_num_samples} timed samples, stopping.")
                break
        # =================================================================
        ...
        if denoise_timesteps > 0:
            ...
            for _, t in enumerate(noise_scheduler.timesteps): #5，4，3，2，1
                ...
                noise_pred_uncond, noise_pred_cond = admm_denoiser(latents, **denoiser_kwargs, **uncond), \
        ...
        if measure_feature_quality and clean_bev is not None:
            ...
            with torch.no_grad():
                ...
                gt_max_abs = gt.reshape(B, -1).abs().max(dim=1).values.clamp(min=1e-8)  # [B]
                eps = 1e-12
                per_sample_psnr = 10.0 * torch.log10((gt_max_abs ** 2) / (per_sample_mse + eps))  # [B]

                # ---- per-sample cosine similarity ----
                # 把每个样本的特征展平成长向量再算 cosine
                pred_flat = pred.reshape(B, -1)
                gt_flat = gt.reshape(B, -1)
                per_sample_cos = torch.nn.functional.cosine_similarity(pred_flat, gt_flat, dim=1)  # [B]
    ...
    if rank == 0:  # 只在主进程打印
        ...
        if measure_feature_quality and fq_count > 0:
            ...
            print(f'[Table 7] MSE  (lower)     = {avg_mse:.4f}     <-- 填入 Table 7 第 2 列')
            print(f'[Table 7] PSNR (higher dB) = {avg_psnr:.2f}    <-- 填入 Table 7 第 3 列')
            print(f'[Table 7] Cos  (higher)    = {avg_cos:.4f}     <-- 填入 Table 7 第 4 列')
            print('=' * 50)
        ...
        if enable_timing:
            ...
            print(f'[Table 8] threshold value   = {regularization_state["threshold"]:.4f}')
            print(f'[Table 8] num_admm_iters    = {regularization_state["num_admm_iters"]:.0f}')
