...
def train():
    ...
    if args.use_up_down_sample and (args.pretrained_admm_denoiser_checkpoint is not None) and (os.path.isfile(args.pretrained_admm_denoiser_checkpoint) or os.path.isdir(args.pretrained_admm_denoiser_checkpoint)):
        admm_denoiser.from_pretrained(args.pretrained_admm_denoiser_checkpoint, subfolder="admm_denoiser")
    ...
    def get_condition(batch):
        ...
        if force_uncond or np.random.rand() < args.uncond_prob:
            denoiser_module = admm_denoiser.module if hasattr(admm_denoiser, "module") else admm_denoiser
            if isinstance(denoiser_module, ADMMDenoiser):
                if 'obj_class' in denoiser_module.layout_encoder.used_condition_types:
                    cond['obj_class'] = torch.ones_like(cond['obj_class']).fill_(denoiser_module.layout_encoder.num_classes_for_layout_object - 1)
                    cond['obj_class'][:, 0] = denoiser_module.layout_encoder.num_classes_for_layout_object - 2
    ...
    if args.resume_from_checkpoint:
        ...
        if os.path.isfile(bev_resume_path):
            ...
            logger.warning(f"No BEV model checkpoint found at {bev_resume_path}; using --bev_checkpoint weights")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(admm_denoiser):
                ...
                # Predict the noise residual and compute loss
                model_pred = admm_denoiser(noisy_latents, timesteps, sqrt_alpha_t=sqrt_alpha_t, **cond)
                ...
                admm_state = accelerator.unwrap_model(admm_denoiser).get_regularization_state()
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(admm_denoiser.parameters(), args.max_grad_norm)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    ...
                    with torch.no_grad():
                        ...
                        eval_results = evaluate(admm_denoiser=admm_denoiser.module,
                                                bev_model=bev_model,
...
def parse_args():
    ...
    parser.add_argument('--pretrained_admm_denoiser_checkpoint', 
                        default=None,
                        help='checkpoint file')
