...
# ===== V2-Version 已废弃：被下方 V3 取代。以下整段以字符串封存保留，不参与运行 =====
_V2_DEPRECATED_SOURCE = '''
class ADMMDenoiser(nn.Module):  # 总共有三种图：精细图，稀疏图和残缺图
    def __init__(
        self,
        in_channels: int = 256,
    ...
    # =====================================================
'''
# ↑↑↑ V2-Version 封存结束（以上整段以字符串保留，不参与运行）↑↑↑


# V3-Version（已废弃：固定相对稀疏阈值的 mini 对照实验未筛出有效 c，保留为历史）
# 相较各版本的区别：
#   · vs ori（最强基座）：唯一实质改动 —— 软阈值的 threshold 由“可学习标量 gamma”
#     改为“固定的相对阈值 sparsity_coef * mean(|Rv|)”（mean 已 detach，不回传梯度）。
#     目的：把“稀疏强度”变成一个可显式控制、可扫描的旋钮；且相对阈值对 R 的整体
#     缩放不敏感（R 放大/缩小时阈值同比例变，置零比例不变 → 无法靠缩放规避阈值）。
#     其余与 ori 逐项一致：单个可学习标量 rho（裸参数）、共享的一套
#     R_struct / R_struct_inverse 权重、zero-init 的 out_layer 输出
#     （final = out_layer(x_0)）、迭代次数 num_admm_iters。
#   · vs v1：不使用残差输出、不使用 softplus、不使用 gamma/rho 形式的阈值。
#   · vs v2：不使用 per-iter 独立 prior 权重、不使用 per-iter rho/gamma、
#     不需要“末轮闭式更新”（共享权重不会触发 DDP unused-parameter）。
#   · sparsity_coef 默认 0.0：此时 threshold=0，软阈值退化为恒等（零稀疏），
#     即“无稀疏对照”。不显式声明该系数时本实验旋钮静默、不改变行为。
class ADMMDenoiserV3Deprecated(nn.Module):  # 总共有三种图：精细图，稀疏图和残缺图
    def __init__(
        self,
        in_channels: int = 256,
        out_channels: int = 256,
        ...
        """
        super(ADMMDenoiserV3Deprecated, self).__init__()

        self.use_up_down_sample = False
        self.num_admm_iters = num_admm_iters
        # 固定相对阈值系数 c：threshold = sparsity_coef * mean(|Rv|)。
        # 非可学习常数；=0 时不产生稀疏（恒等），需显式传入才生效。
        ...
        # ori 风格：单个可学习标量 rho（裸参数，与 ori 一致，不做 softplus）。
        self.rho = nn.Parameter(torch.tensor([0.1]), requires_grad=True)

        # ori 风格：共享的一套 prior 变换权重。
        self.R_struct = RStructTransform(in_channels=256, hidden_channels=512)
        self.R_struct_inverse = RStructInverseTransform(
    ...
    def get_regularization_state(self):
        # 训练/测试循环会调用本方法用于日志；不打印、只返回。
        return {
            "rho": float(self.rho.detach().reshape(-1)[0].item()),
            "sparsity_coef": float(self.sparsity_coef),
            "sparsity": float(self.last_sparsity),
            "num_admm_iters": float(self.num_admm_iters),
        }
    ...
    def forward(
        ...
        last_scheduler_iter=False,
        **kwargs,
    ):  # 模型输入x：已经稀疏化后的图像（Fx），本质是学会从 Fx_gt 到 x_gt 的映射。
        if last_scheduler_iter:
            self.reset_history()
        ...
        u = torch.zeros_like(x_0)  # 拉格朗日乘子
        sparsity_list = []
        # ========== ADMM迭代（共享一套 prior 权重；单 rho）==========
        for i in range(self.num_admm_iters):
            # 1. x-子问题：数据保真项更新（闭式解）
            x_0 = (2 * sqrt_alpha_t * h - u + rho * z) / (2 * alpha_bar_t + rho)
            ...
            # ===== 唯一改动：固定相对阈值软收缩 =====
            # threshold = c * mean(|Rv|)，mean 在 batch 内逐样本求并 detach（不回传梯度）。
            # c=0 时 thr=0 → 软阈值退化为恒等（零稀疏，对照组）。
            if self.sparsity_coef > 0:
                thr = (
                    self.sparsity_coef
                    * Rv.abs().mean(dim=(1, 2, 3), keepdim=True).detach()
                )
            else:
                thr = 0.0
            w_sparse = torch.sign(Rv) * torch.clamp(torch.abs(Rv) - thr, min=0)
            ...
            # ===== inverse transform =====
            z = self.R_struct_inverse(w_sparse, emb_combined, layout_outputs)
            ...
            # 真实稀疏度：被软阈值精确置零的比例
            sparsity = (torch.abs(w_sparse) < 1e-8).float().mean().item()
            sparsity_list.append(sparsity)
        ...
        # ========== 最终输出（与 ori 一致：zero-init out_layer）==========
        final_x0 = x_0
        final_output = self.out_layer(final_x0)
...
# Ori-Version（当前启用版本）
# 目的：恢复最初、目前实测最强的 ADMM-Diff ori 结构，并兼容
# /root/autodl-tmp/ckpts/admm-diff/tiny-ori/checkpoint-50000。
# 相较 V3：
#   1) 撤销固定相对阈值 sparsity_coef * mean(|Rv|)，恢复 ori 的可学习裸参数 gamma；
#   2) 保持 downsample_blocks / upsample_blocks 为空，匹配 tiny-ori checkpoint；
#   3) 保持共享的一套 R_struct / R_struct_inverse、单 rho、zero-init out_layer 输出。


# ===== V4-L1 / ori-best 版本已废弃：以下源码封存保留，不参与运行 =====
# 说明：这是本文件修改前最后一个生效的 ADMMDenoiser。这个就是ori版本。
# 特征：z-update 使用 L1 soft-threshold 稀疏先验：
#   Rv = self.R_struct(v, emb_combined, layout_outputs)
#   w_sparse = sign(Rv) * clamp(abs(Rv) - gamma, min=0)
#   z = self.R_struct_inverse(w_sparse, emb_combined, layout_outputs)
# 该版本保留在字符串中，便于后续实验对照和回滚。
_V4_L1_DEPRECATED_SOURCE = r'''
class ADMMDenoiser(nn.Module):  # 总共有三种图：精细图，稀疏图和残缺图
    def __init__(
        self,
        in_channels: int = 256,
        ...
        self.use_up_down_sample = False
        self.num_admm_iters = num_admm_iters
        # 为兼容训练日志接口保留字段；ori 不使用该实验性旋钮。
        self.sparsity_coef = float(sparsity_coef)

        # tiny-ori checkpoint 没有 down/up adapter 权重；tiny 训练也不启用该路径。
    ...
    def get_regularization_state(self):
        return {
            "rho": float(self.rho.detach().reshape(-1)[0].item()),
            "gamma": float(self.gamma.detach().reshape(-1)[0].item()),
            "threshold": float(self.gamma.detach().reshape(-1)[0].item()),
            "sparsity_coef": float(self.sparsity_coef),
            "sparsity": float(self.last_sparsity),
            "num_admm_iters": float(self.num_admm_iters),
        }
    ...
    def forward(
        ...
        **kwargs,
        ...
        for i in range(self.num_admm_iters):
            x_0 = (
                2 * sqrt_alpha_t * h - u + self.rho * z
            ) / (2 * alpha_bar_t + self.rho)
            v = x_0 + u / (self.rho + 1e-8)
            ...
            Rv = self.R_struct(v, emb_combined, layout_outputs)
            w_sparse = torch.sign(Rv) * torch.clamp(torch.abs(Rv) - self.gamma, min=0)
...
# ===== V5-L2 正则版（当前生效）=====
# 目的：只替换 z-update 的先验形式，验证“L1 稀疏先验是否伤害 BEV dense feature”。
# 与 V4-L1 保持一致的部分：
#   1) 数据保真 x-update、u-update、ADMM 迭代次数不变；
#   2) R_struct / R_struct_inverse / layout_encoder / time_embed / out_layer 全部不变；
#   3) 输出仍保持 ori-best 的 final_output = self.out_layer(final_x0)，不使用残差输出；
#   4) save/load/timing/history 接口不变，训练脚本无需修改。
# 唯一实质改动：
#   L1: w = sign(Rv) * clamp(abs(Rv) - gamma, min=0)
#   L2: shrink = rho / (rho + gamma), w = shrink * Rv
# 含义：L2 只做连续能量收缩，不再硬清零弱响应。
class ADMMDenoiser(nn.Module):  # 总共有三种图：精细图，稀疏图和残缺图
    def __init__(
        self,
        in_channels: int = 256,
    ...
    def get_regularization_state(self):
        rho = self.rho.detach().reshape(-1)[0]
        gamma = self.gamma.detach().reshape(-1)[0]
        shrink = rho / (rho + gamma + 1e-8)
        return {
            "rho": float(rho.item()),
            "gamma": float(gamma.item()),
            # L2 版不再使用 threshold 做硬阈值；保留该字段仅用于兼容旧日志/打印代码。
            "threshold": float(gamma.item()),
            "shrink": float(shrink.item()),
            "sparsity_coef": float(self.sparsity_coef),
            "sparsity": float(self.last_sparsity),
            "num_admm_iters": float(self.num_admm_iters),
        }
    ...
    def forward(
        ...
        **kwargs,
        ...
        for i in range(self.num_admm_iters):
            ...
            # V5-L2: 将 L1 soft-threshold 稀疏先验替换为 L2 proximal shrink。
            # 只做连续能量收缩，不再把小幅值 BEV 特征硬置零。
            rho = self.rho
            gamma = self.gamma
            shrink = rho / (rho + gamma + 1e-8)
            w_l2 = shrink * Rv

            z = self.R_struct_inverse(w_l2, emb_combined, layout_outputs)
            ...
            # 兼容训练日志字段：L2 版不产生真实稀疏，这里只记录近零比例作为诊断量。
            near_zero_ratio = (torch.abs(w_l2) < 1e-6).float().mean().item()
            sparsity_list.append(near_zero_ratio)
