...
class DiffEpochBasedRunner(EpochBasedRunner):
    ...
    def run_iter(self, data_batch, train_mode, **kwargs):
        if self.batch_processor is not None:
            ...
            outputs = self.model.train_step(
                ...
                bev_diffuser=self.bev_diffuser,
                progress=1.0 * (self.epoch + 1) / self.max_epochs,
                **kwargs
            )
        else:
    ...
    def train(self, data_loader, **kwargs):
        if self.bev_diffuser and self.bev_diffuser.auto_denoise_timesteps:
            ...
            self.bev_diffuser.denoise_timesteps = int(
                max_step - (max_step - min_step) * (1.0 * self.epoch / self.max_epochs)
            )
            self.bev_diffuser.num_inference_steps = min(
                self.bev_diffuser.num_inference_steps,
                self.bev_diffuser.denoise_timesteps,
            )
