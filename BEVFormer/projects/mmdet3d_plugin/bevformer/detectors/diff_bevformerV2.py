...
class DiffBEVFormerV2(BEVFormerV2):
    def train_step(
        self, data, optimizer, model_target=None, bev_diffuser=None, progress=None
    ):
    ...
    def forward_pts_train(
        ...
        **kwargs,
        ...
        outs = self.pts_bbox_head(pts_feats, img_metas, prev_bev, given_bev=bev)
        ...
        if bev_diffuser:
            ...
            def get_classifier_gradient(x):
                ...
                x_in = x_in.permute(0, 2, 3, 1).reshape(
                    -1,
                    self.pts_bbox_head.bev_h * self.pts_bbox_head.bev_w,
                    bev.shape[-1],
                )
                outs = model_target.pts_bbox_head(
                    pts_feats, img_metas, prev_bev=prev_bev, given_bev=x_in
                )
            ...
            bev_ = bev_.reshape(
                -1, self.pts_bbox_head.bev_h, self.pts_bbox_head.bev_w, bev.shape[-1]
            ).permute(0, 3, 1, 2)
            bev_ = bev_diffuser(bev_, get_condition(), grad_fn=get_classifier_gradient)
            bev_ = bev_.permute(0, 2, 3, 1).reshape(
                -1, self.pts_bbox_head.bev_h * self.pts_bbox_head.bev_w, bev.shape[-1]
            )
    ...
    def _parse_losses_mix(self, losses, weight=0.5):
        ...
        total_loss = sum(_value for _key, _value in log_vars.items() if "loss" in _key)
        bev_loss = sum(
            _value
