...
# from projects.bevdiffuser.model_utils import build_admmdenoiser
from projects.bevdiffuser.scheduler_utils import DDIMGuidedScheduler
from projects.bevdiffuser.layout_diffusion.admm_denoiser_mynet_ import ADMMDenoiser


class BEVDiffuser(nn.Module):
    def __init__(
        self,
        admmdenoiser_checkpoint_dir=None,
        ...
        self.admmdenoiser = ADMMDenoiser()
        assert admmdenoiser_checkpoint_dir is not None
        self.admmdenoiser.from_pretrained(
            admmdenoiser_checkpoint_dir, subfolder="admm_denoiser"
        )
        self.admmdenoiser.requires_grad_(False)
    ...
    def get_uncondition(self, cond):
        uncond = {}
        if "obj_class" in self.admmdenoiser.layout_encoder.used_condition_types:
            uncond["obj_class"] = torch.ones_like(cond["obj_class"]).fill_(
                self.admmdenoiser.layout_encoder.num_classes_for_layout_object - 1
            )
            uncond["obj_class"][:, 0] = (
                self.admmdenoiser.layout_encoder.num_classes_for_layout_object - 2
            )
        if "obj_name" in self.admmdenoiser.layout_encoder.used_condition_types:
            uncond["obj_name"] = cond["default_obj_names"]
        if "obj_bbox" in self.admmdenoiser.layout_encoder.used_condition_types:
            uncond["obj_bbox"] = torch.zeros_like(cond["obj_bbox"])
            if self.admmdenoiser.layout_encoder.use_3d_bbox:
                uncond["obj_bbox"][:, 0] = torch.FloatTensor(
                    [0, 0, 0, 1, 1, 1, 0, 0, 0]
                )
            else:
    ...
    def forward(self, x, condition=None, grad_fn=None):
        ...
        if self.denoise_timesteps > 0:
            ...
            for _, t in enumerate(self.noise_scheduler.timesteps):
                ...
                sqrt_alpha_t = torch.sqrt(self.noise_scheduler.alphas_cumprod[t])
                sqrt_alpha_t = (
                ...
                noise_pred_uncond, noise_pred_cond = self.admmdenoiser(
                    x, **denoiser_kwargs, **uncond
                ), self.admmdenoiser(x, **denoiser_kwargs, **cond)
                noise_pred = noise_pred_uncond + 2.0 * (
                    noise_pred_cond - noise_pred_uncond
                )
                classifier_gradient = (
                    grad_fn(x) if self.use_classifier_guidence and grad_fn else None
                )
