...
class BEVDiffuser(nn.Module):
    def __init__(self,
        ...
        self.admmdenoiser.from_pretrained(admmdenoiser_checkpoint_dir, subfolder="admm_denoiser")
    ...
    def get_uncondition(self, cond):
        ...
        if 'obj_class' in self.admmdenoiser.layout_encoder.used_condition_types:
            uncond['obj_class'] = torch.ones_like(cond['obj_class']).fill_(self.admmdenoiser.layout_encoder.num_classes_for_layout_object - 1)
            uncond['obj_class'][:, 0] = self.admmdenoiser.layout_encoder.num_classes_for_layout_object - 2
    ...
    def forward(self, x, condition=None, grad_fn=None): 
        ...
        if self.denoise_timesteps > 0:
            ...
            for _, t in enumerate(self.noise_scheduler.timesteps):
                ...
                sqrt_alpha_t = sqrt_alpha_t.view(-1, 1, 1, 1).expand(x.shape[0], 1, 1, 1).to(x.device)
                ...
                noise_pred_uncond, noise_pred_cond = self.admmdenoiser(x, **denoiser_kwargs, **uncond), \
