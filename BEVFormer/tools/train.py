...
def main():
    ...
    if hasattr(cfg, 'plugin'):
        if cfg.plugin:
            ...
            custom_train_model = importlib.import_module(f"{_module_path}.bevformer.apis.train").custom_train_model
    # set cudnn_benchmark
    ...
    if args.admmdenoiser_checkpoint_dir is not None and (osp.isfile(args.admmdenoiser_checkpoint_dir) or osp.isdir(args.admmdenoiser_checkpoint_dir)):
        if cfg.bev_diffuser_cfg:
            cfg.bev_diffuser_cfg.admmdenoiser_checkpoint_dir = args.admmdenoiser_checkpoint_dir
    ...
    if cfg.bev_diffuser_cfg is not None:
        model_target = copy.deepcopy(model) #此时的model_target是DiffBEVFormer（bevformer的子类，待训练模型）
        if cfg.load_from:
            load_checkpoint(model_target, cfg.load_from, map_location='cpu') #这里加载的是bevformer的预训练
            cfg.load_from = None
