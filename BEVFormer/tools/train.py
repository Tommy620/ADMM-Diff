...
from BEVFormer.tools.admm_denoiser import BEVDiffuser
...
def parse_args():
    ...
    parser.add_argument(
        "--admmdenoiser-checkpoint-dir", help="the checkpoint file of admmdenoiser"
    )
    ...
    parser.add_argument("--tracker-project-name", type=str, default="DiffBEVFormer")
...
def main():
    ...
    if hasattr(cfg, "plugin"):
        if cfg.plugin:
            ...
            custom_train_model = importlib.import_module(
                f"{_module_path}.bevformer.apis.train"
            ).custom_train_model
    # set cudnn_benchmark
    ...
    if args.admmdenoiser_checkpoint_dir is not None and (
        osp.isfile(args.admmdenoiser_checkpoint_dir)
        or osp.isdir(args.admmdenoiser_checkpoint_dir)
    ):
        if cfg.bev_diffuser_cfg:
            cfg.bev_diffuser_cfg.admmdenoiser_checkpoint_dir = (
                args.admmdenoiser_checkpoint_dir
            )
    ...
    model.init_weights()  # model就是DiffBEVFormer,就是一个bevformer套了一层训练脚本的子类

    if not cfg.get("train_task_decoder", True):  # 不执行
    ...
    datasets = [build_dataset(cfg.data.train, default_args=dataset_default_args)]
    if len(cfg.workflow) == 2:
        ...
        datasets.append(build_dataset(val_dataset, default_args=dataset_default_args))
    ...
    if cfg.bev_diffuser_cfg is not None:
        model_target = copy.deepcopy(
            model
        )  # 此时的model_target是DiffBEVFormer（bevformer的子类，待训练模型）
        if cfg.load_from:
            load_checkpoint(
                model_target, cfg.load_from, map_location="cpu"
            )  # 这里加载的是bevformer的预训练
            cfg.load_from = None
            if cfg.model.get("pretrained", None) is None:  # 不执行
                cfg.load_from = os.path.join(
                    cfg.info_root, "r101_dcn_fcos3d_pretrain.pth"
                )
                # for bevformerv2
                # cfg.load_from = os.path.join(cfg.info_root,"fcos_r50_coco_2mmdet.pth")
        model_target.requires_grad_(False)
        bev_diffuser = BEVDiffuser(**cfg.bev_diffuser_cfg)  # Unet结构及设置好的调度器
