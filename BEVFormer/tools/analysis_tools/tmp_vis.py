...
def lidiar_render_gt_simple(sample_token, out_path=None):
    """
    ...
    visualize_sample(
        ...
        gt_annotations,
        pred_annotations,
        savepath=out_path if out_path else f"gt_bev_{sample_token}.png",
    )
...
if __name__ == "__main__":
    # 初始化nuScenes
    nusc = NuScenes(
        version="v1.0-mini", dataroot="/root/autodl-fs/nuscenes", verbose=True
    )

    # 选择第一个scene的第一个sample

    sample_token = "3e8750f331d7499e9b5123e9eb70f2e2"

    # 生成GT-only LiDAR俯视图
    output_path = "/tmp/pycharm_project_55/BEVFormer/tools/vis_doc/gt_lidar_bev.png"

    # 使用方案1
    lidiar_render_gt_simple(sample_token, out_path=output_path)

    # 或者使用方案2（更可控）
    # render_gt_lidar_bev(nusc, sample_token, out_path=output_path, axes_limit=50)

    print(f"GT LiDAR BEV image saved to: {output_path}")
# 这个代码成功的得到了lidar俯视图
