    ...
    def get_uncondition(self, cond):
        ...
        if "obj_class" in self.unet.layout_encoder.used_condition_types:
            uncond["obj_class"] = torch.ones_like(cond["obj_class"]).fill_(
                self.unet.layout_encoder.num_classes_for_layout_object - 1
            )
            uncond["obj_class"][:, 0] = (
                self.unet.layout_encoder.num_classes_for_layout_object - 2
            )
        ...
        if self.denoise_timesteps > 0:
            ...
            for _, t in enumerate(self.noise_scheduler.timesteps):
                ...
                noise_pred_uncond, noise_pred_cond = (
                    self.unet(x, t_batch, **uncond)[0],
                    self.unet(x, t_batch, **cond)[0],
                )
